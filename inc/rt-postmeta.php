...
if ( ! class_exists( 'RT_Postmeta' ) ) {
	class RT_Postmeta {
		...
		public function load_styles_and_scripts() {
			...
			wp_enqueue_style( 'jquery-timepicker', $this->base_url . 'assets/css/jquery.timepicker.css', [], $this->version );
			wp_enqueue_style( 'select2', $this->base_url . 'assets/css/select2.min.css', [], $this->version );
			wp_enqueue_style( 'rt-posts-style', $this->base_url . 'assets/css/style.css', [], $this->version );
		...
		public function register_meta_boxes() {
			foreach ( $this->metaboxes as $metabox_id => $args ) {
				add_meta_box(
					$metabox_id,
					$args['title'],
					empty( $args['callback'] ) ? [ $this, 'display_metaboxes' ] : $args['callback'],
					$args['post_type'],
					$args['context'],
					$args['priority'],
					$args['callback_args']
